...
cst=1.0
for _ in range(10):
    model_u, sigma, X_t, Y_t=train_d(n, learning_rate=5e-4, N_epoch=80, print_every=20, batch_size=32)
    with torch.no_grad():
        H_u = np.zeros(N) 
        H_v = np.zeros(N)
        R_u = np.zeros(N)
        ...
        for i in range(len(m_list)):
            ...
            for k in range(N): 
                ...
                for j in range(100):
                        ...
                        mmd_XZ = mmdGST(X_t, Z_temp, model_u, n, sigma, cst, device, dtype)[0] 
                        mmd_YZ = mmdGST(Y_t, Z_temp, model_u, n, sigma, cst, device, dtype)[0]
                        stat.append(float(mmd_XZ - mmd_YZ))
                stat = np.sort(stat)
                thres = stat[94]
                Z1, Z2 = gen_fun2_t(m)
                mmd_XZ = mmdGST(X_t, Z1, model_u, n, sigma, cst, device, dtype)[0] 
                mmd_YZ = mmdGST(Y_t, Z1, model_u, n, sigma, cst, device, dtype)[0] 
                H_u[k] = mmd_XZ - mmd_YZ < 0.0
                R_u[k] = mmd_XZ - mmd_YZ < thres
                P_u[k] = np.searchsorted(stat, float(mmd_XZ - mmd_YZ), side="left")/100.0
                mmd_XZ = mmdGST(X_t, Z2, model_u, n, sigma, cst, device, dtype)[0] 
                mmd_YZ = mmdGST(Y_t, Z2, model_u, n, sigma, cst, device, dtype)[0] 
