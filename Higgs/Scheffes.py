...
def train(model, total_S, total_labels, validation_S, validation_labels,
    ...
    for epoch in trange(load_epoch, epochs+load_epoch):
        ...
        print(loss)
        if epoch % save_per == 0:
            path = './Scheffe/checkpoint%d/'%n+str(epoch)+'/'
            try:
                os.makedirs(path) 
            except:
                pass
            ...
            plt.show()
            plt.savefig('./Scheffe/checkpoint%d/'%n+'loss.png')
...
if __name__ == "__main__":
    ...
    if title == 'test':
        print('-------------------test--------------------')
        path = './Scheffe/checkpoint%d/'%n+str(test_epoch)+'/'
        model = DN().to(device)
        ...
        model = train(model, total_S, total_labels, validation_S, validation_labels,
                     batch_size=batch_size, lr=0.05, epochs=1000, load_epoch=0, save_per=50)
