...
os.environ["CUDA_VISIBLE_DEVICES"]= RFM_config.train_param_configs['gpu_id'] # specify which GPU(s) to be used
...
if __name__ == "__main__":
    ...
    n_org_list = RFM_config.train_param_configs['n_tr_list']
    repeats = RFM_config.train_param_configs['repeats']
    n_tr_list = []
    for n in n_org_list:
        for i in range(repeats):
            n_tr_list.append(n+i)

    batch_size = RFM_config.train_param_configs['batch_size']
    N_epoch = RFM_config.train_param_configs['N_epoch']
    checkpoints_path = RFM_config.expr_configs['checkpoints_path']
    ...
    for n_tr in n_tr_list:
        ...
        try: #make dir
            ...
            pass
        M, explode_flag = rfm(train_loader, test_loader, iters=N_epoch, loader=True, classif=False, 
