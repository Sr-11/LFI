...
class Model(torch.nn.Module):
    def __init__(self, median_heuristic_flag=True, X_heu=None, Y_heu=None):
        ...
        if median_heuristic_flag:
            with torch.no_grad():
                ...
                print('median_heuristic: ', self.sigmaOPT, self.sigma0OPT)  
        else:
    ...
    def update_dist_matrix(self, X, Y, Z, xy=False, xz=False, yz=False, xx=False, yy=False):
        ...
        if xy == True:
            if self.Dxy.shape != (X.shape[0], Y.shape[0]):
                self.Dxy = torch.empty((X.shape[0], Y.shape[0])).to(device)
            if self.Dxy_org.shape != (X.shape[0], Y.shape[0]):
                self.Dxy_org = torch.empty((X.shape[0], Y.shape[0])).to(device)
            Pdist2_(self.Dxy, self.model(X), self.model(Y))
            Pdist2_(self.Dxy_org, self.another_model(X), self.another_model(Y))
        if xz == True:
            if self.Dxz.shape != (X.shape[0], Z.shape[0]):
                self.Dxz = torch.empty((X.shape[0], Z.shape[0])).to(device)
            if self.Dxz_org.shape != (X.shape[0], Z.shape[0]):
                self.Dxz_org = torch.empty((X.shape[0], Z.shape[0])).to(device)
            Pdist2_(self.Dxz, self.model(X), self.model(Z))
            Pdist2_(self.Dxz_org, self.another_model(X), self.another_model(Z))
        if yz == True:
            if self.Dyz.shape != (Y.shape[0], Z.shape[0]):
                self.Dyz = torch.empty((Y.shape[0], Z.shape[0])).to(device)
            if self.Dyz_org.shape != (Y.shape[0], Z.shape[0]):
                self.Dyz_org = torch.empty((Y.shape[0], Z.shape[0])).to(device)
            Pdist2_(self.Dyz, self.model(Y), self.model(Z))
            Pdist2_(self.Dyz_org, self.another_model(Y), self.another_model(Z))
        if xx == True:
            if self.Dxx.shape != (X.shape[0], X.shape[0]):
                self.Dxx = torch.empty((X.shape[0], X.shape[0])).to(device)
            Pdist2_(self.Dxx, self.model(X), self.model(X))
            if self.Dxx_org.shape != (X.shape[0], X.shape[0]):
                self.Dxx_org = torch.empty((X.shape[0], X.shape[0])).to(device)
            Pdist2_(self.Dxx_org, self.another_model(X), self.another_model(X))
        if yy == True:
            if self.Dyy.shape != (Y.shape[0], Y.shape[0]):
                self.Dyy = torch.empty((Y.shape[0], Y.shape[0])).to(device)
            Pdist2_(self.Dyy, self.model(Y), self.model(Y))
            if self.Dyy_org.shape != (Y.shape[0], Y.shape[0]):
                self.Dyy_org = torch.empty((Y.shape[0], Y.shape[0])).to(device)
    ...
    def update_gram_matrix(self, X, Y, Z, xy=False, xz=False, yz=False, xx=True, yy=True):
        if xy == True:
            ...
            self.Kxy *= 1-self.ep
            self.Kxy += self.ep
            self.Kxy *= (self.Dxy_org/self.sigma).neg_().exp_()
            self.Kxy *= self.cst
        if xz == True:
            self.Kxz = (self.Dxz/self.sigma0).neg_().exp_()
            self.Kxz *= (1-self.ep)
            self.Kxz += self.ep
            self.Kxz *= (self.Dxz_org/self.sigma).neg_().exp_()
            self.Kxz *= self.cst
        if yz == True:
            self.Kyz = (self.Dyz/self.sigma0).neg_().exp_()
            self.Kyz *= (1-self.ep)
            self.Kyz += self.ep
            self.Kyz *= (self.Dyz_org/self.sigma).neg_().exp_()
            self.Kyz *= self.cst
        if xx == True:
            self.Kxx = (self.Dxx/self.sigma0).neg_().exp_()
            self.Kxx *= (1-self.ep)
            self.Kxx += self.ep
            self.Kxx *= (self.Dxx_org/self.sigma).neg_().exp_()
            self.Kxx *= self.cst
        if yy == True:
            self.Kyy = (self.Dyy/self.sigma0).neg_().exp_()
            self.Kyy *= (1-self.ep)
            self.Kyy += self.ep
            self.Kyy *= (self.Dyy_org/self.sigma).neg_().exp_()
            self.Kyy *= self.cst
    ...
    def compute_gamma(self, X_te, Y_te, pi, batch_size=10000):
        ...
        for i_monte in range(MAX_LOOPS):
            ...
            # T_XYX = (torch.sum(self.Kxy)-torch.sum(self.Kxx)) / (batch_size**2)
            # mmd2_val, mmd_var = h1_mean_var_gram(self.Kxx, self.Kyy, self.Kxy, is_var_computed=False)
            # gamma_records[i_monte] = T_XYX + pi/2*mmd2_val
        ...
        # mmd2_val, mmd_var, KK = self.compute_MMD(XY_te, is_var_computed=False)
        return gamma
