...
if __name__ == "__main__":
    ...
    chekpoints_mid_path = config.expr_configs['checkpoints_path']
    ...
    for n_tr in ns:
        ...
        for i in trange(num_models):
            n_tr_label = n_tr+i
            folder_path = sys.path[0]+chekpoints_mid_path+'/checkpoints n_tr=%d/'%n_tr_label
        ...
        if test_soft:
            np.save(sys.path[0]+pval_mid_path+'/n_tr=%d_soft.npy'%n_tr, p_soft_mat)
        if test_hard:
            np.save(sys.path[0]+pval_mid_path+'/n_tr=%d_hard.npy'%n_tr, p_hard_mat)
        gc.collect()
        torch.cuda.empty_cache()
