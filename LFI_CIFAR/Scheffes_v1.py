...
n=256
m_list = list(range(4, 180, 4))

F1 = './data/result_SCH_'+str(n)+'_.txt'
F2 = './data/result_LBI_'+str(n)+'_.txt'
...
if True:
    ...
    for m in m_list:
        ...
        for i in range(1000):
            X1, Y1 = gen_fun1(m)
            H[0].append(Scheffe(model, X1))
        thres = np.percentile(H[0], 5)
        H=[[], []]
        T=[[], []]
        for i in range(1000):
            X1, Y1 = gen_fun1(m)
            H[0].append(Scheffe(model, X1)>=thres)
            H[1].append(Scheffe(model, Y1)<thres)
            T[0].append(Scheffe(model, X1)>0.5)
            T[1].append(Scheffe(model, Y1)<0.5)
    ...
    for m in m_list:
        ...
        for i in range(1000):
            X1, Y1 = gen_fun1(m)
            H[0].append(likelihood(model, X1))
        thres = np.percentile(H[0], 5)
        H=[[], []]
        T=[[], []]
        for i in range(1000):
            X1, Y1 = gen_fun1(m)
            H[0].append(likelihood(model, X1)>=thres)
            H[1].append(likelihood(model, Y1)<thres)
            T[0].append(likelihood(model, X1)>0.5)
            T[1].append(likelihood(model, Y1)<0.5)
        st1="n, m= "+str(n)+str('  ')+str(m)+" --- P(max|Z~X):  "+str(np.mean(T[0]))
        st2="n, m= "+str(n)+str('  ')+str(m)+" --- P(max|Z~Y):  "+str(np.mean(T[1]))
        st3="n, m= "+str(n)+str('  ')+str(m)+" --- P(95|Z~X):  "+str(np.mean(H[0]))
        st4="n, m= "+str(n)+str('  ')+str(m)+" --- P(95|Z~Y):  "+str(np.mean(H[1]))
