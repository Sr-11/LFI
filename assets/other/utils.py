...
class Kernel(torch.nn.Module):
    ...
    def compute_loss(self, XY_tr, require_grad=True, **kwargs):
        mmd_val, mmd_var, K = compute_MMD(self, XY_tr, require_grad=require_grad)
        STAT_u = mmd_val / torch.sqrt(mmd_var+10**(-8)) 
        torch.set_grad_enabled(prev)
        return -STAT_u
    def compute_loss_sig(self, XY_tr, require_grad=True, **kwargs):
        mmd_val2, mmd_var, K = compute_MMD(self, XY_tr, require_grad=require_grad)
    ...
    def compute_gamma(self, X_te, Y_te, pi):
        ...
        mmd2_val, mmd_var, K = compute_MMD(self, XY_tr, require_grad=require_grad, is_var_computed=False)
...
def get_error_from_evaluated_scores(X_score, Y_score, pi, gamma=None, verbose = False):
    ...
    type_2_error = scipy.stats.norm.cdf( - np.sqrt(m)*(Mix_mean-gamma)/std)
